...
public class TouchImageView extends AppCompatImageView {
    ...
    private static final int WHAT_CLICK = 0;
    ...
    private boolean longClickReture, isMove;

    // Scale mode on DoubleTap
    private boolean zoomToOriginalSize = true;
    ...
    public void setZoomToOriginalSize(boolean zoomToOriginalSize) {
        ...
        super.setOnClickListener(zoomToOriginalSize ? null : mClickListener);
        super.setOnLongClickListener(zoomToOriginalSize ? null : mLongClickListener);
    }
            ...
            public boolean onTouch(View v, MotionEvent rawEvent) {
                ...
                switch (event.getAction() & MotionEvent.ACTION_MASK) {
                        ...
                        if (xDiff < CLICK && yDiff < CLICK) {
                            ...
                            if (pressTime - lastPressTime <= DOUBLE_PRESS_INTERVAL) {
                                ...
                                if (!longClickReture) {
                                    ...
                                    mClickTimer.schedule(new ClickTask(), 300);
                                }
                            }
                    ...
                    case MotionEvent.ACTION_MOVE:
                        allowInert = false;
                        xDiff = (int) Math.abs(event.getX() - start.x);
                        yDiff = (int) Math.abs(event.getY() - start.y);

                        isMove = xDiff >= CLICK || yDiff >= CLICK;
    ...
    public boolean pagerCanScroll() {
        return mode == NONE && saveScale == minScale;
    }
    ...
    static class TimeHandler extends Handler {
        ...
        TimeHandler(TouchImageView view) {
            ...
            mService = new WeakReference<>(view);
        }

        @Override
        public void handleMessage(Message msg) {
            TouchImageView touchImageView = mService.get();
            if (msg.what == WHAT_CLICK) {
                touchImageView.performClick();
                if (touchImageView.mClickListener != null) {
                    touchImageView.mClickListener.onClick(touchImageView);
                }
            } else if (msg.what == WHAT_LONG_CLICK) {
                touchImageView.performLongClick();
                if (touchImageView.mLongClickListener != null) {
                    touchImageView.longClickReture = touchImageView.mLongClickListener.onLongClick(touchImageView);
                }
            }
        }
    }
