...
public class SocketClient {
    ...
    private void __i__onTimeTick() {
        ...
        SocketHeartBeatHelper heartBeatHelper = socketConfigure.getHeartBeatHelper();

        if (heartBeatHelper != null && heartBeatHelper.isSendHeartBeatEnabled()) {
            if (currentTime - getLastSendHeartBeatMessageTime() >= heartBeatHelper.getHeartBeatInterval()) {
                __i__sendHeartBeat();
        ...
        if (socketPacketHelper != null && socketPacketHelper.isReceiveTimeoutEnabled()) {
            if (currentTime - getLastReceiveMessageTime() >= socketPacketHelper.getReceiveTimeout()) {
                disconnect();
            }
        }

        if (socketPacketHelper != null && socketPacketHelper.isSendTimeoutEnabled()
                && getLastSendMessageTime() != NoSendingTime) {
            if (currentTime - getLastSendMessageTime() >= socketPacketHelper.getSendTimeout()) {
                disconnect();
            }
        }
    }
        ...
        public void run() {
            ...
            try {
                while (self.isConnected()
                    ...
                    SocketPacketHelper socketPacketHelper = self.getSocketConfigure().getSocketPacketHelper();
                    if (socketPacketHelper == null) {
                        continue;
                    }

                    byte[] headerData = socketPacketHelper.getSendHeaderData();
                    int headerDataLength = headerData == null ? 0 : headerData.length;

                    byte[] trailerData = socketPacketHelper.getSendTrailerData();
                    int trailerDataLength = trailerData == null ? 0 : trailerData.length;

                    byte[] packetLengthData = socketPacketHelper.getSendPacketLengthData(packet.getData().length + trailerDataLength);
                    ...
                    try {
                        ...
                        if (packet.getData().length > 0) {
                            ...
                            if (socketPacketHelper.isSendSegmentEnabled()) {
                                segmentLength = Math.min(segmentLength, socketPacketHelper.getSendSegmentLength());
                            }
